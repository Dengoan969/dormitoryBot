...
namespace DomitoryBot.Commands.SubscriptionsService
{
    public class AdminSubscriptionsCommand : IExecutableCommand
    {
        ...
        public async Task Execute(long chatId)
        {
            ...
            {
                await dialogManager.Value.BotClient.SendTextMessageAsync(chatId, "У тебя пока нет созданных рассылок ._.");
            }
            else
            {
                await dialogManager.Value.BotClient.SendTextMessageAsync(chatId, $"Твои рассылки:/n{string.Join("/n", subscriptions)}");
            }
